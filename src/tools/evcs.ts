...
export function registerEvcsTools(server: McpServer): void {
  server.registerTool(
    'victron_evcs_status',
    {
      ...
      inputSchema: {
        host: z.string().describe('EVCS device IP address (connect directly to the EVCS, not the GX)'),
        port: z.number().default(502).describe('Modbus TCP port'),
        unitId: z.number().default(1).describe('Modbus unit ID (default: 1 for EVCS)'),
      },
